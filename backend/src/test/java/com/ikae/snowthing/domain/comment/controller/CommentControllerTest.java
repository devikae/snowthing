...
class CommentControllerTest {
    ...
    void createComment_success() throws Exception {
        CommentCreateRequest request = CommentCreateRequest.builder()
            .content("통합 테스트 댓글 내용")
            .isAnonymous(false)
            .build();

        mockMvc.perform(post("/api/v1/posts/" + post.publicId() + "/comments").with(csrf())
    ...
    void deleteComment_success() throws Exception {
        CommentResponse comment = commentService.createComment(post.publicId(), CommentCreateRequest.builder()
            .content("삭제될 댓글")
            .isAnonymous(false)
            .build(), userDetails, "127.0.0.1");

        mockMvc.perform(delete("/api/v1/comments/" + comment.commentId()).with(csrf()))
            .andExpect(status().isOk())
