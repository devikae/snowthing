...
public class PostRepositoryCustomImpl implements PostRepositoryCustom {
    ...
    public CursorPageResponse<PostListResponse> findPostsByOffset(PostSearchRequest request) {
        ...
        if (total == 0 || offset >= total) {
            return new CursorPageResponse<>(
                    List.of(),
                    PageInfo.ofOffset(page, totalPages, total, false, size)
            );
        }
        ...
        return new CursorPageResponse<>(
                content,
                PageInfo.ofOffset(page, totalPages, total, hasNext, size)
        );
    }
    ...
    public CursorPageResponse<PostListResponse> findPostsByCursor(PostSearchRequest request) {
        ...
        List<PostListResponse> content = resultPosts.stream()
                .map(PostListResponse::from)
                .toList();
        ...
        return new CursorPageResponse<>(
                content,
                PageInfo.ofCursor(nextCursor, hasNext, size)
        );
    }
    ...
    private BooleanExpression cursorCondition(SortType sortType, CursorValue cursor) {
        ...
        if (sortType == SortType.POPULAR) {
            ...
            return post.likeCount.lt(cursor.likeCount().intValue())
                    .or(post.likeCount.eq(cursor.likeCount().intValue()).and(post.id.lt(cursor.id())));
        } else {
            // 최신순 커서: id < lastId
