...
-- 0. 기존 Spike 데이터 초기화
DELETE c FROM `comment` c JOIN `post` p ON p.`post_id` = c.`post_id`
WHERE p.`public_id` IN ('post-spike-distributed-998', 'post-spike-hotspot-999');
DELETE FROM `post` WHERE `public_id` IN ('post-spike-distributed-998', 'post-spike-hotspot-999');
...
ON DUPLICATE KEY UPDATE
    `public_id` = IF(`public_id` = VALUES(`public_id`), `public_id`, NULL);
...
INSERT INTO `post` (`public_id`, `member_id`, `category_id`, `title`, `content`, `writer_ip`, `is_anonymous`, `comment_count`, `created_at`, `updated_at`)
VALUES ('post-spike-distributed-998', @spike_member_id, @spike_category_id, 'Spike [시나리오 A] 분산 1,000건 테스트 글', '내용', '127.0.0.1', FALSE, 1000, NOW(), NOW());
SET @spike_distributed_post_id = LAST_INSERT_ID();

-- Post 999: 시나리오 B (집중 핫스팟 1,000건용)
INSERT INTO `post` (`public_id`, `member_id`, `category_id`, `title`, `content`, `writer_ip`, `is_anonymous`, `comment_count`, `created_at`, `updated_at`)
VALUES ('post-spike-hotspot-999', @spike_member_id, @spike_category_id, 'Spike [시나리오 B] 핫스팟 500건 집중 테스트 글', '내용', '127.0.0.1', FALSE, 1000, NOW(), NOW());
SET @spike_hotspot_post_id = LAST_INSERT_ID();
...
BEGIN
    ...
    WHILE root_idx <= 100 DO
        ...
        VALUES (@spike_distributed_post_id, @spike_member_id, NULL, CONCAT('루트 댓글 #', root_idx), '127.0.0.1', FALSE, FALSE, NOW() + INTERVAL root_idx SECOND, NOW());
        ...
        WHILE reply_idx <= 9 DO
            INSERT INTO `comment` (`post_id`, `member_id`, `parent_id`, `content`, `writer_ip`, `is_anonymous`, `is_deleted`, `created_at`, `updated_at`)
            VALUES (@spike_distributed_post_id, @spike_member_id, current_root_id, CONCAT('대댓글 #', reply_idx, ' (부모:', current_root_id, ')'), '127.0.0.1', FALSE, FALSE, NOW() + INTERVAL (root_idx * 10 + reply_idx) SECOND, NOW());
...
BEGIN
    ...
    WHILE root_idx <= 500 DO
        INSERT INTO `comment` (`post_id`, `member_id`, `parent_id`, `content`, `writer_ip`, `is_anonymous`, `is_deleted`, `created_at`, `updated_at`)
        VALUES (@spike_hotspot_post_id, @spike_member_id, NULL, CONCAT('루트 댓글 #', root_idx), '127.0.0.1', FALSE, FALSE, NOW() + INTERVAL root_idx SECOND, NOW());
    ...
    WHILE reply_idx <= 500 DO
        INSERT INTO `comment` (`post_id`, `member_id`, `parent_id`, `content`, `writer_ip`, `is_anonymous`, `is_deleted`, `created_at`, `updated_at`)
        VALUES (@spike_hotspot_post_id, @spike_member_id, hotspot_root_id, CONCAT('핫스팟 대댓글 #', reply_idx), '127.0.0.1', FALSE, FALSE, NOW() + INTERVAL (500 + reply_idx) SECOND, NOW());
        SET reply_idx = reply_idx + 1;
    END WHILE;
END$$
DELIMITER ;
...
FROM `comment` 
WHERE `post_id` IN (@spike_distributed_post_id, @spike_hotspot_post_id)
GROUP BY `post_id`;
