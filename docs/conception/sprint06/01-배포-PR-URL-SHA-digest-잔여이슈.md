...
2. `stable-<SHA>` 지정은 기본 브랜치 병합 후 실행해야 합니다. 현재 롤백 증거는 보존 중인 `release-<SHA>` 두 개로 만들었습니다.
3. RDS 자동 백업 설정과 운영 연결은 완료됐지만, 백업을 별도 DB로 복원해 읽기 전용 검증 앱으로 확인하는 절차는 아직 남았습니다.
4. GitHub Actions가 사용하는 일부 액션에서 Node.js 20 사용 중단 경고가 발생합니다. 배포 실패는 아니지만 공식 액션의 새 버전을 확인해 갱신해야 합니다.
5. 최초 프런트와 백엔드 배포 사이에 API가 잠시 `502 Bad Gateway`를 반환했습니다. 재현 로그가 없어 원인을 단정하지 않았습니다. 다음 배포부터 컨테이너 상태와 Nginx 오류 로그를 같은 시각에 수집해야 합니다.
