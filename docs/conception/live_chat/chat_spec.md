...
- **보관 정책**: 운영 정책은 3개월이며 월별 일수 차이를 흡수하도록 Logback `maxHistory = 100`으로 설정한다. 일별·100MB 단위로 압축 회전하고 100일이 지난 파일은 자동 삭제한다. 호스트 디스크 고갈을 막기 위해 전체 로그 상한은 2GB로 둔다. 접속·종료 이벤트가 비정상적으로 급증해 100일 안에 2GB를 넘으면 오래된 파일이 기간보다 먼저 정리될 수 있으므로 사용량 경보와 외부 보관이 필요하다.
...
- 백엔드는 직접 연결 주소가 로컬 루프백인 경우에만 Nginx가 덮어쓴 `X-Real-IP`와 `X-Forwarded-For`를 신뢰한다. 외부 주소에서 백엔드로 직접 들어온 요청은 전달 헤더를 무시해 IP 위조를 막는다. 전달 체인은 오른쪽부터 검사해 신뢰 프록시 구간을 제외한 첫 주소를 실제 사용자 IP로 선택한다.
- 감사 로그는 비동기 큐가 가득 차더라도 채팅 요청 스레드를 기다리게 하지 않는다. 서비스 가용성을 우선하는 대신 폭주 구간의 감사 로그 일부가 버려질 수 있으므로 큐 사용량과 파일 수집 상태를 운영 지표로 감시한다.
- 부하 테스트 클라이언트도 운영 설정과 같은 송신·수신 10초 heartbeat를 실제 스케줄러로 전송한 뒤 연결 생존 여부를 확인한다.
