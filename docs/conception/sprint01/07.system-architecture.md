...
flowchart TB
    ...
    subgraph Cache_Layer [🚀 In-Memory Session & Cache Layer]
        Redis[(🧠 Redis 7.x In-Memory<br/>Spring Session Redis / SADD Voters<br/>INCR Counter / AOF Persistence)]
    end
    ...
    ReactionSvc -->|In-Memory INCR & SADD| Redis
...
* **주요 역할**:
  ...
  * `SADD`: 중복 투표 방지 인메모리 검증.
  * `INCR`: DB 락을 잡지 않는 인메모리 카운팅.
