...
- **라이브톡 백엔드 운영 배포 실패 원인 수정 (2026-09-21)**:
  ...
  - 신규 서버에는 기존과 같이 실제 IP 복원 설정을 생성하고, 기존 운영 서버에서는 현재 설정을 유지하는 멱등 배포 방식입니다.
- **운영 라이브챗 WebSocket 연결 복구 (2026-09-21)**:
  - PR #23 배포 후 비회원 최근 채팅 조회 API가 `200 OK`를 반환하는 것을 확인했지만, `wss://snowthing.org/ws-chat` 핸드셰이크는 응답 없이 시간 초과되는 상태를 확인했습니다.
