...
- **익명 게시글 목록 작성자 표기 복원 (2026-09-13)**:
  ...
  - 오르카 브라우저에서 익명 목록 10건이 `익명 (127.0.***.***)` 형식으로 표시되는 것을 확인했습니다. 로컬 요청이므로 루프백 주소가 표시되며 운영에서는 복원된 실제 방문자 IP가 마스킹됩니다.
  - 대상 페이지 ESLint는 오류 없이 통과했고 기존 `<img>` 최적화 경고 1건만 남았습니다. `npm run build`도 성공했습니다.
  - 운영 Nginx 설정의 저장소 반영과 IP fallback 정책 변경은 UI 브랜치에 섞지 않고 `deploy/aws-ec2` 또는 별도 백엔드 작업에서 처리합니다.

- **공통 내비게이션 활성 상태 디자인 수정 (2026-09-13)**:
  - 익명게시판에만 적용되던 빨간 글자와 빨간 점을 제거하고, 모든 메뉴가 같은 방식으로 현재 위치를 표시하도록 통일했습니다.
  - 데스크톱은 남색 글자, 옅은 하늘색 배경, 2px 하늘색 하단 선을 사용하고 모바일은 같은 색상과 상단 안쪽 선을 사용합니다.
  - 현재 메뉴 링크에 `aria-current="page"`를 추가해 화면뿐 아니라 보조 기술에서도 현재 위치를 구분할 수 있게 했습니다.
  - `npm run build`와 대상 컴포넌트 ESLint를 통과했습니다. 기존 프로필 이미지의 `<img>` 최적화 경고 1건은 이번 변경과 무관하게 남아 있습니다.
  - 오르카 브라우저의 익명게시판 화면에서 빨간 점과 강조 클래스가 사라지고, 남색 글자·옅은 하늘색 배경·2px 하단 선이 실제 적용되는 것을 확인했습니다.

- **게시글 글쓰기·상세 화면 Modern Alpine 재설계 (2026-09-13)**:
  - 세 번째 Stitch 전달본의 글쓰기·상세 시안을 기준으로 기존 게시글 작성, 상세 조회, 추천·비추천, 수정·삭제, 댓글·대댓글 로직을 유지하면서 화면 구조와 스타일을 재구성했습니다.
  - 요청에 따라 글쓰기 제목 옆 `REAL ANONYMOUS` 배지와 제목 아래 개인정보 보호 안내 문구를 제거했습니다. 오르카 브라우저 DOM에서도 두 문구가 존재하지 않는 것을 확인했습니다.
  - 글쓰기 화면에 게시판 분류, 작성자/익명 비밀번호, 제목, Toast UI 본문, 이미지 URL, 취소·등록 영역을 배치하고 우측 설질 카드는 추후 연동을 전제로 한 예시임을 표시했습니다.
  - 상세 화면에 목록 이동, 게시글 메타데이터, 작성자와 권한 버튼, 본문·이미지, 추천·비추천·주소 복사, 익명 댓글 작성 및 기존 댓글 계층을 배치했습니다. 우측 카풀·동행 카드는 현재 화면용 예시로 구분했습니다.
  - 오르카 내장 브라우저에서 비로그인 익명글을 실제 등록해 `/posts/70789531-d20b-484e-b63f-a0a5e316b8ee` 상세 이동, 작성한 제목·본문, 댓글 입력 영역과 카풀 카드 렌더링을 확인했습니다.
  - 변경 파일 ESLint는 오류 없이 통과했고 기존 상세 이미지의 `<img>` 최적화 권고 1건만 남았습니다. `npm run build`도 성공했습니다.

- **UI 전용 브랜치 게시판 목록 화면 재설계 (2026-09-12)**:
  - 두 번째 Stitch 시안(`stitch_winter_sports_community_main (1)/code.html`, `DESIGN.md`, `screen.png`)을 기준으로 중고장터를 제외한 전체글·자유게시판·익명게시판·장비/테크닉·리조트 맛집 목록 화면을 하나의 공통 구조로 재설계했습니다.
  - 기존 게시글 목록 API, 카테고리 필터, 검색, 페이지 이동, 삭제·차단 게시글 처리 로직은 유지하고 게시판별 제목·설명·공지·핫 토픽만 달라지도록 구성했습니다. 서버가 아직 제공하지 않는 추천순·댓글순·조회순은 동작하는 것처럼 보이지 않도록 비활성화했습니다.
  - 별도 구현되어 있던 `/posts/anonymous`는 `/posts?category=ANONYMOUS`로 연결해 동일한 목록·검색·페이지 이동 로직을 사용하도록 정리했습니다.
  - 공통 헤더와 모바일 하단 메뉴의 게시판 활성 상태를 확장했고, 요청에 따라 우측 카풀·동행 위젯을 모든 대상 게시판에 유지했습니다.
  - Docker의 기존 MySQL 영속 볼륨과 로컬 백엔드를 실행한 뒤 오르카 내장 브라우저 642px 화면에서 익명게시판과 자유게시판의 게시판별 문구·공지·토픽 전환 및 반응형 배치를 확인했습니다. 자유게시판 API 데이터 10건과 2페이지가 실제 목록에 표시되고 로딩·오류 상태가 해제되는 것까지 검증했습니다.
  - `npm run build` 성공. 변경 파일 대상 ESLint는 오류 없이 통과했으며 기존 `<img>` 사용에 대한 성능 권고 2건만 남아 있습니다.

- **UI 전용 브랜치 메인 화면 재설계 (2026-09-11)**:
  - 제공된 Stitch 시안(`code.html`, `DESIGN.md`, `screen.png`)을 기준으로 메인 화면을 Modern Alpine 커뮤니티 스타일로 변경했습니다.
  - 공통 2단 헤더, 통합 검색, 글쓰기·로그인 상태, 데스크톱 메뉴와 모바일 하단 고정 메뉴를 `SiteChrome`에 반영해 전체 페이지에서 사용하도록 했습니다.
  - 메인에 시즌 배너, 리조트 설질 카드, 고밀도 게시글 목록, 장비 관리, 익명 게시판, 카풀·동행, 중고장터 영역을 구성했습니다.
  - 아직 API가 없는 설질·카풀·중고장터·프로모션은 화면 검증용 목업 데이터이며, 실제 기능과 API는 이후 연결해야 합니다.
  - `npm run build` 성공. 브라우저에서 1231px 데스크톱 화면의 배너·가로 스크롤·게시판 8:4 배치와 공통 헤더를 확인했습니다.
  - `npm run lint`는 기존 `ToastEditor`, `ToastViewer`, 글쓰기·게시판 페이지의 규칙 위반 6건 때문에 실패했습니다. 이번 변경 파일에는 `img` 최적화 권고만 남아 있습니다.
...
- **댓글 동시 삭제 카운트 중복 차감 방지 (2026-09-06)**:
  - 삭제를 `is_deleted = false` 조건부 UPDATE로 원자화했습니다.
  - 영향 행이 1인 최초 삭제 요청만 `post.commentCount`를 차감하며, 동시 요청에서 영향 행이 0이면 카운트를 차감하지 않습니다.

- **Sprint 03 로그인 익명 댓글 삭제 UI 권한 정합성 보강 (2026-09-06)**:
  - 댓글 조회 응답에 요청자 기준 `canDelete`, `requiresDeletePassword`를 추가했습니다. 로그인 익명 작성자의 내부 식별자는 `ownerPublicId`로만 판정하고 `@JsonIgnore`로 외부 응답에서 제외했습니다.
  - 일반 회원 본인, 로그인 익명 본인, 비회원 익명, 관리자, 타 회원, 삭제 댓글을 삭제 권한 매트릭스와 동일하게 판정하며 루트 프리뷰와 분리 대댓글에도 적용했습니다.
  - 프런트엔드의 `writer` 기반 `isOwnerAnonMember` 추정을 제거하고, 삭제 버튼과 비밀번호 입력 여부를 각각 서버의 `canDelete`, `requiresDeletePassword`로 결정하도록 변경했습니다.
  - `CommentDeleteTest`에 작성자별 권한 매트릭스와 분리 대댓글 권한 테스트를, `CommentControllerTest`에 로그인 익명 작성자의 응답 권한 및 내부 식별자 비노출 테스트를 추가했습니다.
  - 검증 결과: `CommentDeleteTest`, `CommentReadTest`, 백엔드 `spotlessCheck`, 변경 프런트 파일 ESLint, `npm run build`가 통과했습니다. 기존 게시글 이미지의 `<img>` 관련 ESLint 경고 1건은 남아 있습니다.
- **댓글 삭제 후 대댓글 페이징 상태 초기화 (2026-09-06)**:
  - 삭제 성공 후 루트 댓글을 재조회할 때 기존 `replyPagingByRootId`도 함께 초기화해 이전 커서·더보기 상태가 새 미리보기와 섞이지 않도록 수정했습니다.

- **댓글 조회 실패 상태 및 재시도 UI 보강 (2026-09-06)**:
  - 최초 댓글 조회의 로딩·성공 빈 목록·실패 상태를 분리해 서버 오류가 빈 목록처럼 표시되지 않도록 수정했습니다.
  - 최초 조회 실패 시 오류 메시지와 재시도 버튼을 표시하고, 재시도 중에는 중복 요청을 막는 로딩 상태를 적용했습니다.
  - 댓글 더보기 실패 시 이미 조회한 댓글을 유지하면서 오류 메시지와 더보기 재시도 버튼을 표시하도록 구현했습니다.
  - HTTP 비정상 응답도 실패로 처리하며 서버 오류 메시지가 없으면 기본 안내 문구를 사용하도록 보강했습니다.
  - 검증 결과: 변경 파일 ESLint는 오류 없이 통과했으며 기존 `<img>` 최적화 경고 1건만 남았습니다. `npm run build`도 통과했습니다.

- **대댓글 생성 락 순서 및 조회 비용 개선 (2026-09-06)**:
  - 요청된 부모 댓글은 일반 조회로 식별하고, 최상위 루트 댓글 한 행에만 비관적 쓰기 락을 획득하도록 락 지점을 단일화했습니다.
  - 루트의 활성 대댓글 ID 전체를 잠그고 애플리케이션에서 개수를 세던 `findActiveReplyIdsForUpdate()`를 제거하고, 루트 락을 보유한 트랜잭션 안에서 `countActiveReplies()`를 실행하도록 변경했습니다.
  - 같은 루트의 서로 다른 대댓글을 부모로 지정한 두 동시 요청이 모두 데드락 없이 저장되는 회귀 테스트를 추가했습니다.
  - 기존 99개 상한 경계 동시성 테스트를 포함한 `CommentCreateTest`와 백엔드 `spotlessCheck`가 통과했습니다.
- **댓글 식별자·커서 프론트 정밀도 보호 (2026-09-06)**:
  - 댓글 응답의 `Long` 식별자와 커서를 JSON 문자열로 직렬화하고 프론트 상태를 `string`으로 변경했습니다.
  - JavaScript `number`의 `MAX_SAFE_INTEGER` 초과 시 댓글 수정·삭제 및 커서가 손상되는 문제를 방지했습니다.
- **댓글 삭제 tombstone 및 카운트 정책 단순화 (2026-09-06)**:
  - 삭제된 루트와 대댓글을 모두 tombstone으로 계속 노출하고, 루트가 모든 자식과 함께 삭제되어도 목록에서 숨기지 않도록 정책을 변경했습니다.
  - `post.commentCount`, `replyCount`, `totalReplyCount`, 프리뷰·대댓글 목록을 삭제 포함 전체 댓글 기준으로 통일했습니다.
  - 대댓글 100개 작성 제한 검증은 삭제 댓글이 한도를 영구 점유하지 않도록 활성 대댓글 수 기준을 유지했습니다.
  - 삭제 시 게시글 댓글 수를 차감하지 않도록 서비스와 테스트를 갱신했습니다.

- **댓글 대댓글 조회 게시글 공개 상태 검증 보강 (2026-09-06)**:
  - `getCommentReplies`가 루트 댓글 여부만 확인하던 문제를 수정하고, 루트 댓글의 게시글을 조회해 삭제 여부와 `PostStatus.NORMAL`을 공통 검증하도록 변경했다.
...
- **게시판 설명 문구 및 실시간 베스트 장식 제거 (2026-09-13)**:
  - 전체 게시판, 자유게시판, 익명게시판, 장비·테크닉, 리조트 맛집의 상단 설명 문구를 화면과 게시판 설정에서 모두 제거했습니다.
  - 메인 게시판의 `실시간 베스트` 앞에 있던 빨간 온천 모양 문자와 빨간색 전용 클래스를 제거했습니다.
  - 오르카 브라우저에서 메인과 다섯 게시판을 각각 열어 온천 문자와 설명 문단이 표시되지 않는 것을 확인했습니다.
  - 대상 파일 ESLint는 오류 없이 통과했고 기존 `<img>` 최적화 경고 6건만 남았습니다. `npm run build`도 성공했습니다.
