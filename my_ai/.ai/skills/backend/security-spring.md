...
- 인증된 사용자만 조회할 수 있는 응답에 `Cache-Control: public`을 적용하지 않는다.
- 공유 프록시나 CDN 캐시를 사용할 때는 다른 사용자의 요청에 응답이 재사용되어도 되는 공개 데이터인지 먼저 확인한다.
- 공개 이미지라면 다운로드 인증을 요구하지 않는 공개 조회 경로로 분리하고, 비공개 이미지라면 `private` 또는 `no-store`와 권한 검사를 유지한다.
- 공개 게시글 이미지와 회원 전용 이미지가 함께 존재하면 URL prefix, CDN behavior, 서명 URL/Cookie, 캐시 정책을 분리한다.
